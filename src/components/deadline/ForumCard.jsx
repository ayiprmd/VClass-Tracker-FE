    ...
    <div className="bg-white rounded-2xl p-5 shadow-xs hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 overflow-hidden border border-slate-200/80 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
          <Icon name="forum" size={24} className="text-blue-600" />
        </div>
        <div className="min-w-0">
          ...
          <p className="text-caption font-caption text-slate-500 truncate">
            {forum.course} {forum.courseCode ? `(${forum.courseCode})` : ''}
          </p>
        </div>
      </div>
      ...
      {/* Right: Action Button */}
      <a
        href={forum.vclassUrl || 'https://v-class.gunadarma.ac.id'}
        target="_blank"
        rel="noopener noreferrer"
        className="px-3.5 py-2 rounded-xl bg-blue-100 hover:bg-blue-600 hover:text-white text-blue-700 text-label-sm font-semibold transition-all duration-200 flex items-center gap-1.5 shrink-0 shadow-2xs hover:scale-105 active:scale-95"
      >
        <span>Buka</span>
